...
from user_service.infrastructure.persistence.user_repo_impl import SqlUserProfileRepository
from user_service.application.commands.update_profile import UpdateProfileCommand, UpdateProfileCommandHandler
from user_service.application.queries.get_profile import GetProfileQuery, GetProfileQueryHandler
...
def get_repo(uow: SqlAlchemyUnitOfWork | None = None) -> SqlUserProfileRepository:
    # If not using a UoW context (e.g. queries), create a temporary session
...
mediator.register(
    ...
    lambda: UpdateProfileCommandHandler(
        uow=(uow := get_uow()),
        repo=get_repo(uow)
    )
)
...
mediator.register(
    GetProfileQuery,
    lambda: GetProfileQueryHandler(
        repo=get_repo()  # Queries manage their own connection/don't need UoW
    )
)
